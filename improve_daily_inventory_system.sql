-- daily_inventory_summary 시스템 개선
-- 뷰를 테이블로 변경하여 HTML에서 날짜별 조회 가능하도록 개선
...
-- 3. 인덱스 생성 (성능 최적화)
CREATE INDEX IF NOT EXISTS idx_daily_inventory_date ON daily_inventory_summary(summary_date);
CREATE INDEX IF NOT EXISTS idx_daily_inventory_part ON daily_inventory_summary(part_number);
CREATE INDEX IF NOT EXISTS idx_daily_inventory_date_part ON daily_inventory_summary(summary_date, part_number);

-- 4. 일별 재고 요약 데이터 생성 함수
...
-- 6. inventory_transactions 변경 시 자동 업데이트 트리거
...
-- 7. inventory 테이블 변경 시 자동 업데이트 트리거
...
-- 10. 생성된 데이터 확인
SELECT 
    'Daily inventory summary system improved successfully!' as status,
...
SELECT 
    ...
    stock_status
FROM daily_inventory_summary 
WHERE summary_date >= CURRENT_DATE - 2
