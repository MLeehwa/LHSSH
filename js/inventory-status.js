...
class InventoryStatus {
    ...
    async loadInventoryData(includeAS = false) {
        ...
        if (baseSnapshotInfo && baseSnapshotInfo.length > 0) {
            ...
            // 반품 합계: return_records
            const totalReturnMap = new Map();
            try {
                const { data: allReturns } = await this.supabase
                    .from('return_records')
                    .select('part_number, quantity')
                    .gte('return_date', calcStartNextStr)
                    .lte('return_date', todayForCalc);
                if (allReturns) {
                    allReturns.forEach(r => {
                        totalReturnMap.set(r.part_number, (totalReturnMap.get(r.part_number) || 0) + (r.quantity || 0));
                    });
                }
            } catch (retErr) {
                console.warn('반품 합계 조회 오류:', retErr);
            }

            // 2-4. 전진 계산: 기준재고 + 총입고 - 총출고 - 총반품 = 현재재고
            result = [...allowedPartNumbers].map(partNumber => {
                ...
                const totalRet = totalReturnMap.get(partNumber) || 0;
                const calculatedStock = baseStock + totalIn - totalOut - totalRet;
        ...
        try {
                ...
                if (todaySeqIds.length > 0) {
                    ...
                    if (todayOutParts) {
                        todayOutParts.forEach(p => {
                            const q = Number(p.actual_qty) || Number(p.scanned_qty) || 0;
                            outboundMap.set(p.part_number, (outboundMap.get(p.part_number) || 0) + q);
                        });
                    }
                }
            }

            // ★ 반품: return_records의 return_date 기준
            const returnMap = new Map();
            try {
                const { data: todayReturns } = await this.supabase
                    .from('return_records')
                    .select('part_number, quantity')
                ...
                if (todayReturns) {
                    todayReturns.forEach(r => {
                        returnMap.set(r.part_number, (returnMap.get(r.part_number) || 0) + (r.quantity || 0));
                    });
                }
            } catch (retErr) {
                console.warn('당일 반품 데이터 조회 오류 (무시 가능):', retErr);
            }

            // 결과에 today_inbound, today_outbound, today_return 추가
            result.forEach(item => {
                item.today_inbound = inboundMap.get(item.part_number) || 0;
                item.today_outbound = outboundMap.get(item.part_number) || 0;
                item.today_return = returnMap.get(item.part_number) || 0;
            });

            console.log(`당일(${todayStr}) 데이터 병합 완료: 입고 ${inboundMap.size}, 출고 ${outboundMap.size}, 반품 ${returnMap.size}개 파트`);
        } catch (todayErr) {
            ...
            result.forEach(item => {
                ...
                item.today_return = item.today_return || 0;
            });
        }
    ...
    async applyFilters() {
        ...
        this.filteredTransactions = this.transactions.filter(transaction => {
            ...
            const transactionDateNorm = typeof rawDate === 'string'
                ? rawDate.split('T')[0]
                : (rawDate ? new Date(rawDate).toISOString().split('T')[0] : '');
            const matchesPartNumber = !partNumberFilter || partNumber.includes(partNumberFilter);
            const matchesDate = !dateFilter || transactionDateNorm === dateFilter;
    ...
    renderInventory() {
        ...
        if (this.filteredInventory.length === 0) {
            ...
            this.filteredInventory.forEach(item => {
                ...
                const todayReturn = item.today_return || 0;
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm ${todayReturn > 0 ? 'text-orange-500 font-semibold' : 'text-gray-400'}">${todayReturn > 0 ? '-' + todayReturn : '0'}</td>
    ...
    async calculateDateStock() {
        ...
        try {
            ...
            if (snapshotData && snapshotData.length > 0) {
                ...
                // ★ 전날 스냅샷이 없을 때 기존 로직은 closing_stock(일마감)을 시작재고로 쓴 뒤 당일 수불을 또 더해 이중 반영됨.
                //    전날 있음: 전일마감 + 당일 수불 = 당일 이론 마감
                //    전날 없음: 스냅샷 closing을 일마감으로 신뢰, 기초는 역산
                this.dateStockData = snapshotData
                    .filter(item => partSet.has(item.part_number))
                    .map(item => {
                        const closing = Number(item.closing_stock) || 0;
                        ...
                        if (prevStock !== undefined && prevStock !== null) {
                            previous_stock = Number(prevStock) || 0;
                            calculated_stock = previous_stock + inbound - outbound + adjustment;
                        } else {
                            calculated_stock = closing;
                            previous_stock = closing - inbound + outbound - adjustment;
                        }
    ...
    async loadDailyInOutData() {
        ...
        try {
            ...
            const startNorm = startDate.split('T')[0];
            const endNorm = endDate.split('T')[0];

            // 2. ★ 기준 스냅샷 = DB상 가장 이른 일자 (재고 현황 loadInventoryData 와 동일)
            //    예전: lte(조회시작일)로 "시작일 이전 마지막 스냅샷"을 쓰면, 스냅샷이 2/1·3/1 두 개일 때
            //    조회 시작이 2/1이면 기준 2/1, 4/1이면 기준 3/1 → 같은 달력일 칼럼 재고가 달라지는 모순 발생.
            //    단일 2/1만 있어도 재고 탭과 일자별 탭의 의미를 맞추기 위해 MIN(snapshot_date)로 통일.
            const { data: minSnapRow } = await this.supabase
                .from('daily_inventory_snapshot')
                .select('snapshot_date')
                .order('snapshot_date', { ascending: true })
                .limit(1);
            ...
            if (baseDate) {
                ...
                console.log(`기준 스냅샷(전역 최초): ${baseDate} (${baseStockMap.size}개 파트)`);
            } else {
                console.warn('스냅샷 데이터 없음 - 재고 0으로 시작');
            }

            // 조회 시작이 기준 스냅샷보다 이르면: 전진은 스냅샷 익일부터만 의미 있음 → 그 전 칼럼은 스냅샷 이전이라 0/미정
            if (baseDate && startNorm < baseDate) {
                console.warn(
                    `[일자별 입출고] 조회 시작(${startNorm})이 최초 스냅샷(${baseDate})보다 이릅니다. ` +
                    `${baseDate} 이전 날짜 칼럼 재고는 계산하지 않습니다(0으로 표시).`
                );
            }

            // 기준 스냅샷 다음날부터 전진 누적(roll) — 단, 입출고 "조회" 시작일은 별도(transStartStr)
            const calcStartDate = baseDate || startNorm;
            const calcStartNext = new Date(calcStartDate + 'T12:00:00');
            calcStartNext.setDate(calcStartNext.getDate() + 1);
            const calcStartNextStr = `${calcStartNext.getFullYear()}-${String(calcStartNext.getMonth()+1).padStart(2,'0')}-${String(calcStartNext.getDate()).padStart(2,'0')}`;

            // ★ 조회 시작일 = 스냅샷일이면 기존 로직은 익일부터만 입출고를 가져와 해당일 컬럼이 전부 0이 됨 → 시작일부터 포함
            // ★ 스냅샷 없음: 전진 계산도 시작일부터 (기존은 익일부터라 첫날 누락)
            let transStartStr;
            if (!baseDate) {
                transStartStr = startNorm;
            } else if (startNorm === baseDate) {
                transStartStr = startNorm;
            } else {
                transStartStr = calcStartNextStr;
            }

            const rollStartStr = baseDate ? calcStartNextStr : transStartStr;

            // 3. 입고 데이터: arn_containers + arn_parts 기준 (transStartStr ~ endNorm)
            const { data: inboundContainers, error: icError } = await this.supabase
                .from('arn_containers')
                .select('arn_number, inbound_date')
                .eq('status', 'COMPLETED')
                .gte('inbound_date', transStartStr)
            ...
            let returnData = [];
            try {
                const { data: returns, error: retError } = await this.supabase
                    .from('return_records')
                    .select('part_number, quantity, return_date')
                    .gte('return_date', transStartStr)
                    .lte('return_date', endNorm);
                if (retError) console.warn('반품 조회 오류:', retError);
                returnData = returns || [];
            } catch (retErr) {
                console.warn('반품 데이터 조회 오류:', retErr);
            }

            console.log(`기준 스냅샷: ${baseDate}, 입출고조회시작: ${transStartStr}, 전진누적시작: ${rollStartStr}, 입고: ${inboundPartsData.length}건, 출고: ${outboundPartsData.length}건, 반품: ${returnData.length}건`);
            ...
            outboundPartsData.forEach(p => {
                ...
                const qty = Number(p.actual_qty) || Number(p.scanned_qty) || 0;
            ...
            returnData.forEach(r => {
                const retDate = typeof r.return_date === 'string' ? r.return_date.split('T')[0] : r.return_date;
                if (!retDate) return;
                const key = `${retDate}|${r.part_number}`;
                dateReturnMap.set(key, (dateReturnMap.get(key) || 0) + (Number(r.quantity) || 0));
            });

            // rollStartStr ~ endNorm 전진 계산용 날짜 목록 (스냅샷 있음: 익일~, 없음: 조회 시작일~)
            const fullDateList = [];
            const fullCurDate = new Date(rollStartStr + 'T12:00:00');
            const fullEndDateObj = new Date(endNorm + 'T12:00:00');
            while (fullCurDate <= fullEndDateObj) {
                const y = fullCurDate.getFullYear();
                const m = String(fullCurDate.getMonth() + 1).padStart(2, '0');
                const dd = String(fullCurDate.getDate()).padStart(2, '0');
                fullDateList.push(`${y}-${m}-${dd}`);
                fullCurDate.setDate(fullCurDate.getDate() + 1);
            }

            // 각 파트에 날짜별 입고/출고/반품 데이터 저장
            fullDateList.forEach(date => {
                partDataMap.forEach((partData, partNumber) => {
                    ...
                    const returnQty = dateReturnMap.get(`${date}|${partNumber}`) || 0;
            ...
            // ★ 스냅샷 기준 전진 계산: 기준일 마감재고에서 시작하여 앞으로 계산
            // 당일 마감재고 = 전날 마감재고 + 당일 입고 - 당일 출고 - 당일 반품
            partDataMap.forEach((partData, partNumber) => {
                let stock = baseStockMap.get(partNumber) || 0;
                fullDateList.forEach(date => {
                    const dateData = partData.dates[date];
                    if (!dateData) return;
                    stock = stock + (dateData.inbound || 0) - (dateData.outbound || 0) - (dateData.returnQty || 0);
                    dateData.stock = stock;
                });
            });

            // 조회 시작일 = 스냅샷일: fullDateList는 익일부터라 당일(스냅샷일) 셀이 비어 있었음 → 당일 입출고 + 스냅샷 마감재고로 보강
            if (baseDate && startNorm === baseDate) {
                partDataMap.forEach((partData, partNumber) => {
                    const inboundQty = dateInboundMap.get(`${baseDate}|${partNumber}`) || 0;
                    const outboundQty = dateOutboundMap.get(`${baseDate}|${partNumber}`) || 0;
                    const returnQty = dateReturnMap.get(`${baseDate}|${partNumber}`) || 0;
                    partData.dates[baseDate] = {
                        inbound: inboundQty,
                        outbound: outboundQty,
                        returnQty: returnQty,
                        stock: baseStockMap.get(partNumber) || 0
                    };
                });
            }
    ...
    renderDailyInOutTable(dateList) {
        ...
        dateList.forEach(date => {
            ...
            dateHeader.innerHTML = `
                ...
                <div class="text-xs text-gray-600 mt-1">
                    ...
                    <span class="text-orange-500">반품</span> | 
        ...
        dateList.forEach(() => {
            ['입고', '출고', '반품', '재고'].forEach((label, idx) => {
                ...
                else if (idx === 2) subHeader.classList.add('text-orange-500');
        ...
        this.dailyInOutData.forEach(partData => {
            ...
            // 각 날짜별 데이터
            dateList.forEach(date => {
                const dateData = partData.dates[date] || { inbound: 0, outbound: 0, returnQty: 0, stock: 0 };
                ...
                returnCell.className = 'px-2 py-3 text-sm text-center text-orange-500';
                returnCell.textContent = dateData.returnQty || 0;
    ...
    async exportDailyInOutExcel() {
        ...
        try {
            ...
            // 반품 구역
            const returnTitleRow = worksheetInOut.addRow(['반품 현황', ...Array(this.dailyInOutDateList.length).fill('')]);
            returnTitleRow.getCell(1).fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: 'FFFFC000' }
            };
            returnTitleRow.getCell(1).font = {
                bold: true,
                size: 12
            };
            returnTitleRow.getCell(1).border = {
                top: { style: 'medium' },
                bottom: { style: 'thin' },
                left: { style: 'thin' },
                right: { style: 'thin' }
            ...
            // 반품 헤더
            const returnHeader = ['파트 번호'];
            this.dailyInOutDateList.forEach(date => {
                returnHeader.push(formatDate(date));
            });
            const returnHeaderRow = worksheetInOut.addRow(returnHeader);
            returnHeaderRow.eachCell((cell) => {
                cell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
            ...
            this.dailyInOutData.forEach((partData, rowIndex) => {
                const row = [partData.part_number];
                this.dailyInOutDateList.forEach(date => {
                    const dateData = partData.dates[date] || { returnQty: 0 };
                    row.push(dateData.returnQty || 0);
                });
                const dataRow = worksheetInOut.addRow(row);
                dataRow.eachCell((cell, colNumber) => {
                    cell.fill = {
                        type: 'pattern',
                        pattern: 'solid',
                        fgColor: { argb: rowIndex % 2 === 0 ? 'FFFCE4D6' : 'FFFFFFFF' }
                    };
                    cell.font = {
                        color: { argb: 'FFED7D31' },
                        size: 10
