-- daily_inventory_summary 뷰를 날짜별로 구분 가능하도록 수정
-- 이 스크립트는 기존 뷰를 삭제하고 날짜별 구분이 가능한 뷰로 재생성합니다

-- 기존 뷰 삭제
DROP VIEW IF EXISTS daily_inventory_summary;

-- 날짜별 구분 가능한 daily_inventory_summary 뷰 생성
CREATE VIEW daily_inventory_summary AS
SELECT 
    CURRENT_DATE as summary_date,  -- 날짜 컬럼 추가
    i.part_number,
    i.current_stock,
    i.status,
    i.min_stock,
    i.max_stock,
    COALESCE(inbound.today_inbound, 0) as today_inbound,
    COALESCE(outbound.today_outbound, 0) as today_outbound,
    i.last_updated,
    -- 추가 통계 정보
    (i.current_stock - COALESCE(outbound.today_outbound, 0) + COALESCE(inbound.today_inbound, 0)) as calculated_stock,
    CASE 
        WHEN i.current_stock <= i.min_stock THEN 'LOW_STOCK'
        WHEN i.current_stock >= i.max_stock THEN 'OVERSTOCK'
        ELSE 'NORMAL'
    END as stock_status
FROM inventory i
LEFT JOIN (
    -- 오늘 입고 수량
    SELECT 
        part_number, 
        SUM(quantity) as today_inbound
    FROM inventory_transactions 
    WHERE transaction_date = CURRENT_DATE 
        AND transaction_type = 'INBOUND'
    GROUP BY part_number
) inbound ON i.part_number = inbound.part_number
LEFT JOIN (
    -- 오늘 출고 수량
    SELECT 
        part_number, 
        SUM(quantity) as today_outbound
    FROM inventory_transactions 
    WHERE transaction_date = CURRENT_DATE 
        AND transaction_type = 'OUTBOUND'
    GROUP BY part_number
) outbound ON i.part_number = outbound.part_number;

-- 날짜별 조회를 위한 함수 생성
CREATE OR REPLACE FUNCTION get_daily_inventory_summary(target_date DATE DEFAULT CURRENT_DATE)
RETURNS TABLE (
    summary_date DATE,
    part_number VARCHAR(50),
    current_stock INTEGER,
    status VARCHAR(20),
    min_stock INTEGER,
    max_stock INTEGER,
    today_inbound INTEGER,
    today_outbound INTEGER,
    last_updated TIMESTAMP WITH TIME ZONE,
    calculated_stock INTEGER,
    stock_status VARCHAR(20)
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        target_date as summary_date,
        i.part_number,
        i.current_stock,
        i.status,
        i.min_stock,
        i.max_stock,
        COALESCE(inbound.today_inbound, 0) as today_inbound,
        COALESCE(outbound.today_outbound, 0) as today_outbound,
        i.last_updated,
        (i.current_stock - COALESCE(outbound.today_outbound, 0) + COALESCE(inbound.today_inbound, 0)) as calculated_stock,
        CASE 
            WHEN i.current_stock <= i.min_stock THEN 'LOW_STOCK'
            WHEN i.current_stock >= i.max_stock THEN 'OVERSTOCK'
            ELSE 'NORMAL'
        END as stock_status
    FROM inventory i
    LEFT JOIN (
        -- 지정된 날짜 입고 수량
        SELECT 
            part_number, 
            SUM(quantity) as today_inbound
        FROM inventory_transactions 
        WHERE transaction_date = target_date 
            AND transaction_type = 'INBOUND'
        GROUP BY part_number
    ) inbound ON i.part_number = inbound.part_number
    LEFT JOIN (
        -- 지정된 날짜 출고 수량
        SELECT 
            part_number, 
            SUM(quantity) as today_outbound
        FROM inventory_transactions 
        WHERE transaction_date = target_date 
            AND transaction_type = 'OUTBOUND'
        GROUP BY part_number
    ) outbound ON i.part_number = outbound.part_number;
END;
$$ LANGUAGE plpgsql;

-- 뷰 생성 확인
SELECT 'daily_inventory_summary 뷰가 날짜별 구분 가능하도록 성공적으로 생성되었습니다.' as status;

-- 함수 사용 예시
SELECT '날짜별 조회 함수 사용법: SELECT * FROM get_daily_inventory_summary(''2024-01-15'');' as usage;

-- 오늘 데이터 확인 (최대 10개 행)
SELECT 
    summary_date,
    part_number,
    current_stock,
    today_inbound,
    today_outbound,
    calculated_stock,
    stock_status
FROM daily_inventory_summary 
ORDER BY part_number 
LIMIT 10;
